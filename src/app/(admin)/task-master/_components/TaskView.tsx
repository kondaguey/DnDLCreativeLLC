                ...
                {completedTasks.map((item) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className={`mt-4 shrink-0 w-5 h-5 rounded border transition-all flex items-center justify-center ${
                        completedSelection.has(item.id)
...
function TaskCard({
  ...
  const progress =
    totalSub === 0
      ? item.status === "completed"
        ? 100
        : 0
      : Math.round((completedSub / totalSub) * 100); // COLOR MIXING LOGIC: Priority + Urgency

  let statusColor = "slate";
  let statusBorder = "border-white/5";
  let statusGlow = "";
  let bgEffect = "bg-slate-900/60"; // 1. BASE PRIORITY THEME
  ...
  const daysUntil = getDaysUntil(item.due_date); // --- COMPACT VIEW RENDER ---
      ...
      >
        ...
        <div className="flex-1 min-w-0 flex flex-col md:grid md:grid-cols-[1fr_auto_auto_auto] md:items-center gap-2">
          ...
          <div className="flex items-center gap-2 overflow-hidden w-full">
            ...
            {isLocked ? (
              <span
                className={`flex-1 font-bold text-sm md:text-base cursor-pointer hover:underline line-clamp-1 ${
                  item.status === "completed"
              ...
              <input
                ...
                onChange={(e) =>
                  onUpdateTitle && onUpdateTitle(item.id, e.target.value)
                }
            ...
            <div className="flex items-center gap-1.5">
              ...
              {totalSub > 0 && (
                <span className="text-[10px] font-mono text-slate-500 bg-black/20 px-1.5 rounded flex items-center gap-1 shrink-0">
                  <CheckSquare size={10} />                  {completedSub}/
                  {totalSub}               {" "}
                </span>
              )}{" "}
              {item.due_date ? (
                <span
                  className={`text-[10px] font-mono ${
                    statusColor === "rose" ? "text-rose-400" : "text-slate-500"
                  }`}
                >
      ...
      <div className="flex items-start gap-3 p-4 md:p-5 w-full relative z-10 flex-1">
        ...
        <div className="flex flex-col gap-2 items-center shrink-0 pt-1">
          ...
          <button
            ...
            className={`w-6 h-6 md:w-5 md:h-5 rounded-full border-2 flex items-center justify-center transition-all shadow-inner ${
              item.status === "completed"
        ...
        <div className="flex-1 min-w-0 flex flex-col h-full">
          ...
          <div className="flex justify-between items-start gap-2 mb-1">
            ...
            {isLocked ? (
              <h4
                className={`font-black text-sm md:text-base leading-tight break-words transition-all cursor-pointer select-none ${
            ...
            <div className="mt-4 flex flex-col gap-2">
              ...
              <RecurringSummary
                ...
                onToggleToday={(id, dateStr) => {
                  const completed =
                    (item.metadata?.completed_dates as string[]) || [];
          ...
          <div className="mt-auto pt-2 flex flex-col md:flex-row md:flex-wrap items-stretch md:items-center gap-2 border-t border-white/5">
            ...
            {item.status === "archived" ? (
              ...
              <>
                ...
                <div
                  className={`flex flex-col md:flex-row md:items-center gap-2 md:gap-2 w-full md:w-auto ${isLocked ? "pointer-events-none opacity-50" : ""}`}
                >
                  {" "}
          ...
          <div
            onClick={() => setExpanded(!expanded)}
            className={`flex items-center gap-1 mt-2 text-[10px] font-bold uppercase tracking-widest cursor-pointer transition-all w-fit rounded-md select-none ${
                ...
                >
                  ...
                  {!isLocked ? (
                    <input
                      ...
                      className={`flex-1 bg-transparent text-sm font-medium focus:outline-none border-b border-transparent focus:border-cyan-500/50 transition-all pt-0.5 ${
                        sub.status === "completed"
                          ? "line-through text-slate-600"
                          : "text-slate-100"
                      }`}
  ...
  return (
    <>
      ...
      <button
        ...
        className={`flex items-center gap-1.5 text-xs md:text-[10px] uppercase font-black px-3 py-2 md:px-2 md:py-1 rounded-xl border transition-all whitespace-nowrap shadow-inner ${statusBg} ${statusBorder} ${statusText} ${
          isOpen
...
function RecurrenceSwitcher({
  ...
  const handleOpen = (e: React.PointerEvent) => {
    ...
    if (triggerRef.current) {
      ...
      const screenHeight = window.innerHeight; // If it overflows the bottom, POP IT UP ABOVE THE BUTTON
  ...
  return (
    <div className="flex items-center justify-between px-1">
      ...
      <button
        ...
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all shadow-md active:scale-95 ${
          isTodayDone
